...
const { req, log, isUrl, downloadIMG, retry, delay } = require("./utils");
...
class OBook {
  ...
  async create() {
    ...
    for (const chapterUrl of chapterUrls) {
      ...
      const content = await retry(this.fetch(chapter.content));
  ...
  async fetch(url) {
    const res = await retry(req("GET", url, this.cookie, null));
    return res.body;
  }
  ...
  async downloadChapterImages(chapter) {
    ...
    for (const image of images) {
      ...
      try {
        await retry(downloadIMG({ url: imageUrl, dest }));
      } catch (error) {
        console.log({ error });
        copyFileSync("./placeholder.png", dest);
      }
    }
  }
